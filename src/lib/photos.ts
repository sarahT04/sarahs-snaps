...
export function getCountryFlag(input: string): string | null {
    ...
    if (values.includes(codeCandidate)) {
        return 'https://flagicons.lipis.dev/flags/4x3/' + codeCandidate.toLowerCase() + '.svg';
    }
 ...
 */
export function photosByCity(photos: Photo[], city: string, country?: string): Photo[] {
 ...
 */
export async function getPhotosByCity(city: string, country?: string): Promise<Photo[]> {
